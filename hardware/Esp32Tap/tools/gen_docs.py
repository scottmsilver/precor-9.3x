#!/usr/bin/env python3
"""Generate Rev E NETLIST, BOM, and optionally CPL files from design.py.
...
with ``--skip-cpl`` while the Rev E PCB is not yet available.
"""
...
def parse_args(arguments: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Esp32Tap Rev E NETLIST.md and BOM.csv; generate the " "placed-board CPL unless --skip-cpl is set."
        )
    )
    parser.add_argument(
        "--skip-cpl",
...
def render_netlist() -> str:
    """Render the human-readable design table and Rev E topology notes."""
    lines = [
    ...
    lines.extend(
        [
            ...
            "* **Harness interfaces:** J1 (CONSOLE) and J2 (MOTOR) are the "
            "same Molex 0441440003/C585890 right-angle SMD 8P8C RJ45 jack, "
            "one per short board edge (Rev E: J1 left, J2 right). There is "
            "no mechanical keying between them; the CONSOLE/MOTOR "
            "silkscreen beside each jack is the only differentiator. J1/J2 "
            "pads 9/10 are the jack's mechanical shell stakes, not "
            "signals.",
            "* **Release status:** "
...
def validate_staged_outputs(
    ...
    if not netlist.startswith("# Esp32Tap Rev E NETLIST"):
