#!/usr/bin/env python3
"""Generate Rev D NETLIST, BOM, and optionally CPL files from design.py.
...
with ``--skip-cpl`` while the Rev D PCB is not yet available.
"""
...
def parse_args(arguments: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Esp32Tap Rev D NETLIST.md and BOM.csv; generate the " "placed-board CPL unless --skip-cpl is set."
        )
    )
    parser.add_argument(
        "--skip-cpl",
...
def render_netlist() -> str:
    """Render the human-readable design table and Rev D topology notes."""
    lines = [
...
def validate_staged_outputs(
    ...
    if not netlist.startswith("# Esp32Tap Rev D NETLIST"):
